...
class LoadAnnotations:
    ...
    def _load_caption(self, results):
        ...
        filename = osp.join(results['seg_prefix'],
                            results['ann_info']['caption'])
        if self.file_client.exists(filename):
            caption_contents = self.file_client.get_text(filename)
            caption_content = caption_contents.split('\n')[0]
        else:
            caption_content = ''
        results['gt_caption'] = caption_content
        return results
