...
Date: 2025-01-14 14:03:15
Description: 
LastEditors: knightdby
LastEditTime: 2025-06-22 10:17:34
FilePath: /MetaSemiDetr/detr_ssod/models/mate_dino_detr_ssod.py
...
class Projector(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv2d(in_channels=256, out_channels=256, kernel_size=(
            3, 3), stride=(1, 1), padding=(1, 1), bias=False)
        self.bn1 = nn.BatchNorm2d(
            256, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
        self.ac1 = nn.ReLU()

        self.conv2 = nn.Conv2d(in_channels=256, out_channels=256, kernel_size=(
            3, 3), stride=(1, 1), padding=(1, 1), bias=False)
...
class MetaDinoDetrSSOD(MultiSteamDetector):
    def __init__(self, model: dict, train_cfg=None, test_cfg=None):
        super(MetaDinoDetrSSOD, self).__init__(
    ...
    def forward_train(self, img, img_metas, **kwargs):
        ...
        if "sup" in data_groups:
            ...
            sup_loss = self.student.forward_train(
                **data_groups["sup"], curr_step=self.curr_step)
    ...
    def foward_unsup_train(self, teacher_data, student_data):
        ...
        student_info['gt_caption'] = student_data['gt_caption']
    ...
    def unsup_loss(
        ...
        **kwargs,
        ...
        all_cls_scores, all_bbox_preds, enc_cls_scores, enc_bbox_preds, dn_cls_scores, dn_bbox_preds = student_info[
            'outs']
        ...
        with torch.no_grad():
            for img_id, (cls_scores, bbox_preds, gt_labels, gt_bboxes) in enumerate(zip(cls_scores_list_, bbox_preds_list_, gt_labels_list_, gt_bboxes_list_)):
                ...
                if num_bboxes == 0 or num_gts == 0:
                    ...
                    match_gt_cost_list.append(
                        cls_scores.new_zeros(0).detach().cpu())
                    match_gt_inds_list.append(
                        cls_scores.new_zeros(0).long().detach().cpu())
                    continue
                ...
                # cls cost
                cls_cost = self.student.bbox_head.assigner2.cls_cost(
                    cls_scores, gt_labels)
                # regression L1 cost
                normalize_gt_bboxes = gt_bboxes / factor
                reg_cost = self.student.bbox_head.assigner2.reg_cost(
                    bbox_preds, normalize_gt_bboxes)
                # regression iou cost, defaultly giou is used in official DETR.
                bboxes = bbox_cxcywh_to_xyxy(bbox_preds) * factor
                iou_cost = self.student.bbox_head.assigner2.iou_cost(
                    bboxes, gt_bboxes)
                ...
                # pos_inds means the matcheding inital gt bbox index
                pos_inds_gmm = matched_col_inds
        ...
        if isinstance(self.train_cfg.pseudo_label_initial_score_thr, float):
            # default set 0.4
            base_thr = self.train_cfg.pseudo_label_initial_score_thr
        else:
        ...
        for img_id, (gt_bboxes, gt_labels, gt_scores, det_bboxes, det_labels, det_scores, match_gt_cost, match_gt_inds) in enumerate(zip(gt_bboxes_list_, gt_labels_list_, gt_scores_list_,
            ...
            valid_inds = torch.nonzero(
                match_gt_cost <= thr_, as_tuple=False).squeeze().unique()
            valid_gt_inds_1 = match_gt_inds[valid_inds]

            valid_gt_inds_2 = torch.nonzero(
                gt_scores >= base_thr, as_tuple=False).squeeze().unique()

            valid_gt_inds = torch.cat(
                (valid_gt_inds_1.to(imgs.device), valid_gt_inds_2.to(imgs.device))).unique()
        ...
        dn_args = (targets_v1, self.student.bbox_head.dn_number,
                   self.student.bbox_head.dn_label_noise_ratio, self.student.bbox_head.dn_box_noise_scale)
        ...
        input_query_label_1, input_query_bbox_1, input_query_label_2, input_query_bbox_2, attn_mask_1, dn_meta_1 = self.prepare_unsup_cdn(
            teacher_info, student_info, pseudo_bboxes_v1, pseudo_labels_v1, det_bboxes_gmm_list, det_labels_gmm_list, dn_args=dn_args)

        # construct the complete dn querys
        input_query_label_v1 = torch.cat(
            [input_query_label_1, input_query_label_2], dim=1)
        input_query_bbox_v1 = torch.cat(
            [input_query_bbox_1, input_query_bbox_2], dim=1)

        # forward with the denoising query

        (hs_v1, outputs_class_v1, outputs_coord_v1, interm_outputs_class_v1, interm_outputs_coord_v1,
            consistency_outputs_class_v1, consistency_outputs_coord_v1, dn_outputs_class_v1, dn_outputs_coord_v1), caption_loss, cap_query_feat_s = \
            self.student.bbox_head.forward_dummy(
                aug_v1_feat, img_metas_v1, input_query_label_v1, input_query_bbox_v1, attn_mask_1, dn_meta_1, gt_caption=student_info['gt_caption'])
        ...
        dn_args = (targets_v2, self.student.bbox_head.dn_number,
                   self.student.bbox_head.dn_label_noise_ratio, self.student.bbox_head.dn_box_noise_scale)

        with torch.no_grad():
            input_query_label_1, input_query_bbox_1, input_query_label_2, input_query_bbox_2, attn_mask_2, dn_meta_2 = self.prepare_unsup_cdn(
                teacher_info, teacher_info, pseudo_bboxes_v2, pseudo_labels_v2, det_bboxes_gmm_list, det_labels_gmm_list, dn_args=dn_args, prior_info=prior_info)

            # construct the complete dn querys
            input_query_label_v2 = torch.cat(
                [input_query_label_1, input_query_label_2], dim=1)
            input_query_bbox_v2 = torch.cat(
                [input_query_bbox_1, input_query_bbox_2], dim=1)
            # forward with the denoising query
            # import ipdb;ipdb.set_trace()
            (hs_v2, outputs_class_v2, outputs_coord_v2, interm_outputs_class_v2, interm_outputs_coord_v2,
                consistency_outputs_class_v2, consistency_outputs_coord_v2, dn_outputs_class_v2, dn_outputs_coord_v2), _, cap_query_feat_t = \
                self.teacher.bbox_head.forward_dummy(
                    aug_v2_feat, img_metas_v2, input_query_label_v2, input_query_bbox_v2, attn_mask_2, dn_meta_2)
        ...
        for layer_id in range(len(hs_v1)):
            ...
            # hs_v1_feats: [bs, seq_len, embed_dim]
            hs_v1_feats, hs_v2_feats = hs_v1[layer_id][:,
                                                       :pad_size, ...], hs_v2[layer_id][:, :pad_size, ...]

            h1 = hs_v1_feats[known_bid.long(), map_known_indice]
            h2 = hs_v2_feats[known_bid.long(), map_known_indice]

            loss = (F.mse_loss(F.normalize(h1, p=2, dim=-1), F.normalize(h2, p=2,
                    dim=-1).detach(), reduction='none') * loss_weights.unsqueeze(-1)).mean()

            losses.update({"consis_loss.d{}".format(layer_id): 10 * loss})

        loss = (F.mse_loss(F.normalize(cap_query_feat_s[0], p=2, dim=-1),
                           F.normalize(
                               cap_query_feat_t[0], p=2, dim=-1).detach(),
                           reduction='none') * loss_weights.unsqueeze(-1)).mean()
        losses.update({"consis_loss.d_cap": 10 * loss})

        return losses

    def prepare_unsup_cdn(self, teacher_info, student_info, pseudo_bboxes, pseudo_labels, det_bboxes, det_laebls,
                          dn_args=None, hidden_dim=256, num_queries=300, num_classes=80, prior_info=None):
        ...
        for img_meta, pseudo_bbox in zip(img_metas_tgt, pseudo_bboxes):
            ...
            if pseudo_bbox.size(0) == 0:
                pseudo_bbox = pseudo_bbox.new_tensor(
        ...
        batch_idx = torch.cat([imgs_tgt.new_full((t.shape[0],), i)
                              for i, t in enumerate(norm_pseudo_bboxes_list)])
        ...
        # if there is no pseudo bbox, we still have a single denoising query
        pad_size_1 = int(single_pad_1 * dn_number_1)
        ...
        if len(known_num):
            map_known_indice_1 = torch.cat(
                [torch.tensor(range(num)) for num in known_num])
            map_known_indice_1 = torch.cat(
                [map_known_indice_1 + single_pad_1 * i for i in range(dn_number_1)]).long()

        input_query_bbox_1[(known_bid_1.long(), map_known_indice_1)
                           ] = consistency_bbox_embed
        ...
        if prior_info is None:
            ...
            loss_weights = batched_loss_weights.unsqueeze(
                -1).repeat(dn_number_1, 1)

            # extract the roi feats
            rois_bboxes = torch.cat(
                [known_bid_1.unsqueeze(-1), batched_proposal_bboxes.repeat(dn_number_1, 1)], dim=-1)

            with torch.no_grad():
                # prepare the content feature
                # extract the backbone feature for multi-level feats
                mlvl_feats = self.teacher.extract_feat(
                    imgs_src)   # [8x, 16x, 32x]
                # get the multi-level feature maps
                mlvl_feats, mlvl_masks, mlvl_pos = self.prepare_feats(
                    mlvl_feats, img_metas_src)

                # convert the bbox into rois
                consistency_query_embed = self.roi_extractor(
                    mlvl_feats, rois_bboxes)  # [num_rois, 256, 7, 7]
            # apply the projector
            consistency_query_embed = self.projector(consistency_query_embed)

            input_query_label_1[(known_bid_1.long(),
                                 map_known_indice_1)] = consistency_query_embed
        ...
        batch_idx = torch.cat([torch.full_like(t.long(), i)
        ...
        if label_noise_ratio > 0:
            ...
            chosen_indice = torch.nonzero(
                p < (label_noise_ratio * 0.5)).view(-1)   # half of bbox prob
        ...
        positive_idx = torch.tensor(range(len(boxes))).long(
        ).cuda().unsqueeze(0).repeat(dn_number_2, 1)
        positive_idx += (torch.tensor(range(dn_number_2)) *
        ...
        if len(known_num):
            map_known_indice_2 = torch.cat(
                [torch.tensor(range(num)) for num in known_num])  # [1,2, 1,2,3]
            map_known_indice_2 = torch.cat(
                [map_known_indice_2 + single_pad_2 * i for i in range(2 * dn_number_2)]).long()
        if len(known_bid_2):
            input_query_label_2[(known_bid_2.long(),
                                 map_known_indice_2)] = input_label_embed
            input_query_bbox_2[(known_bid_2.long(),
                                map_known_indice_2)] = input_bbox_embed
        ...
        for i in range(dn_number_1):
            if i == 0:
                attn_mask[single_pad_1 * i:single_pad_1 *
                          (i + 1), single_pad_1 * (i + 1):(pad_size_1 + pad_size_2)] = True
            else:
                attn_mask[single_pad_1 * i:single_pad_1 *
                          (i + 1), single_pad_1 * (i + 1):(pad_size_1 + pad_size_2)] = True
                attn_mask[single_pad_1 * i:single_pad_1 *
                          (i + 1), :single_pad_1 * i] = True

        # cdn part
        for j in range(dn_number_2):
            if j == dn_number_2 - 1:
                attn_mask[(pad_size_1 + single_pad_2 * 2 * j): (pad_size_1 +
                                                                single_pad_2 * 2 * (j + 1)), :(pad_size_1 + single_pad_2 * j * 2)] = True
            else:
                attn_mask[(pad_size_1 + single_pad_2 * 2 * j): (pad_size_1 + single_pad_2 * 2 * (
                    j + 1)), (pad_size_1 + single_pad_2 * 2 * (j + 1)):(pad_size_1 + pad_size_2)] = True
                attn_mask[(pad_size_1 + single_pad_2 * 2 * j): (pad_size_1 +
                                                                single_pad_2 * 2 * (j + 1)), :(pad_size_1 + single_pad_2 * 2 * j)] = True
    ...
    def prepare_feats(self, mlvl_feats, img_metas):
        ...
        for l, feat in enumerate(mlvl_feats):
            mlvl_masks.append(F.interpolate(
    ...
    def extract_student_info(self, img, img_metas, **kwargs):
        ...
        with torch.no_grad():
            outs, _ = self.student.bbox_head.forward(feat, img_metas)
    ...
    def _fit_gmm(self, data_points, device=None):
        ...
        # from low to high according the cost
        pos_cost_gmm, sort_inds = pos_cost_gmm.sort()
    ...
    def extract_teacher_info(self, img, img_metas, **kwargs):
        ...
        proposal_box_list = [p if p.shape[0] > 0 else p.new_zeros(
            0, 5) for p in proposal_box_list]
        ...
        for proposal_box, proposal_label in zip(proposal_box_list, proposal_label_list):
            ...
            valid_inds = torch.nonzero(
                proposal_box[:, -1] >= pseudo_thr, as_tuple=False).squeeze().unique()

            # filter the empty pseudo labels
            # unnormalized [x1, y1, x2, y2] format
            tmp_bboxes = proposal_box[valid_inds, :4]
