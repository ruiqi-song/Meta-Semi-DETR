...
FilePath: /MetaSemiDetr/detr_od/models/dense_heads/matedino_detr_ssod_head.py
...
class MetaDINODETRSSODHead(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        self.caption_pretrain = caption_pretrain
    ...
    def _init_layers(self):
        ...
        if self.dec_pred_bbox_embed_share:
            ...
            box_embed_layerlist = [copy.deepcopy(_bbox_embed) for i in range(
                self.transformer.num_decoder_layers)]
        if self.dec_pred_class_embed_share:
            class_embed_layerlist = [_class_embed for i in range(
                self.transformer.num_decoder_layers)]
        else:
            class_embed_layerlist = [copy.deepcopy(_class_embed) for i in range(
                self.transformer.num_decoder_layers)]
        ...
        self.label_enc = nn.Embedding(
            self.dn_labelbook_size + 1, self.embed_dims)
    ...
    def loss(self,
             # [num_dec_layer, bs, num_query, cls_out_channels]
             all_cls_scores,
             all_bbox_preds,        # [num_dec_layer, bs, num_query, 4]
             enc_cls_scores,
             enc_bbox_preds,
             # [num_dec_layer, bs, padding_size, cls_out_channels]
             dn_cls_scores,
        ...
        if self.in_warm_up and is_pseudo_label:
            ...
            dn_losses_bbox = [torch.as_tensor(0.).to(
                'cuda') for i in range(num_dec_layers)]
            dn_losses_iou = [torch.as_tensor(0.).to(
                'cuda') for i in range(num_dec_layers)]
            dn_losses_bbox_xy = [torch.as_tensor(0.).to(
                'cuda') for i in range(num_dec_layers)]
