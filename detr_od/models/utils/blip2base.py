...
Date:  2024-06-20 14:57:14
Description: 
LastEditors: Anonymous
LastEditTime: 2025-06-23 09:35:01
FilePath: /MetaSemiDetr/detr_od/models/utils/blip2.py
...
class Blip2Base(BaseModel):
    ...
    def init_tokenizer(cls, truncation_side="right"):
        tokenizer = BertTokenizer.from_pretrained(
            "./weights/bert-base-uncased", truncation_side=truncation_side)
        tokenizer.add_special_tokens({"bos_token": "[DEC]"})
        tokenizer.add_special_tokens(
            {'additional_special_tokens': ['[IMG]', '[OBS]', '[POS]', '<cam>', '</cam>',
                                           '<target>', '<cnt>', '</cnt>',
                                           '<obj>', '</obj>', '</target>'
                                           ]})
        return tokenizer
    ...
    def init_Qformer(cls, num_query_token, vision_width, cross_attention_freq=2):
        ...
        Qformer = BertLMHeadModel.from_pretrained(
            "./weights/bert-base-uncased", config=encoder_config
        )
    ...
    def get_optimizer_params(self, weight_decay, lr_scale=1):
        ...
        lr_scales = list(lr_scale ** (vit_num_layers + 1 - i)
