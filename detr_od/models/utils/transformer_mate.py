...
class ImageQueryCAEncoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None,
        ...
        self.cross_track_attn = MSDeformAttn(
    ...
    def forward(self, tgt, memory,
        ...
        tgt_track = self.cross_track_attn(self.with_pos_embed(output, query_pos).transpose(0, 1),
            ...
            0, 1).contiguous(),
            memory.transpose(
            0, 1), spatial_shapes, level_start_index,
...
class DINOTransformerEncoder(nn.Module):
    def __init__(self, encoder_layer, num_layers,
                 ...
                 num_queries=300, deformable_encoder=True,
...
class DINOTransformerDecoderLayer(nn.Module):
    ...
    def forward_sa(self, tgt: Optional[Tensor],  # nq, bs, d_model
        ...
        # self attention
        if 'encoded_grounding' in extra:
            prompt_tokens = extra['encoded_grounding']
            promot_pos = prompt_tokens.detach().clone()
            prompt_mask = extra['grounding_attention_mask']
            if self_attn_mask is not None:
                ori_size = tgt.shape[0]
                new_mask_size = tgt.shape[0]+prompt_tokens.shape[0]
                new_self_attn_mask = torch.zeros(
                    (tgt.shape[1], new_mask_size, new_mask_size), dtype=torch.bool, device=tgt.device)
                new_self_attn_mask[:, :ori_size, :ori_size] = self_attn_mask.unsqueeze(
                    0).repeat(tgt.shape[1], 1, 1)  # denoising matching keepmask
                new_self_attn_mask[:, :ori_size, ori_size:].transpose(1, 2)[
                    prompt_mask] = True
                new_self_attn_mask[:, ori_size:, :ori_size][prompt_mask] = True
                new_self_attn_mask = new_self_attn_mask.repeat_interleave(
                    self.n_heads, dim=0)
            else:
                ori_size = tgt.shape[0]
                new_mask_size = tgt.shape[0]+prompt_tokens.shape[0]
            ...
            if self.self_attn is not None:
                ...
                tgt2 = self.self_attn(
                    q, k, tgt, attn_mask=new_self_attn_mask)[0]
            ...
            if self.self_attn is not None:
                if self.decoder_sa_type == 'sa':
                    q = k = self.with_pos_embed(tgt, tgt_query_pos)
                    tgt2 = self.self_attn(
                        q, k, tgt, attn_mask=self_attn_mask)[0]
                    tgt = tgt + self.dropout2(tgt2)
                    tgt = self.norm2(tgt)
                else:
        ...
        for funcname in self.module_seq:
            if funcname == 'ffn':
                ...
                tgt = self.forward_sa(tgt, tgt_query_pos, tgt_query_sine_embed,
                                      tgt_key_padding_mask, tgt_reference_points,
                                      memory, memory_key_padding_mask, memory_level_start_index,
                                      memory_spatial_shapes, memory_pos, self_attn_mask, cross_attn_mask, extra=extra)
            else:
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if not self.deformable_decoder and self.modulate_hw_attn:
                ...
                query_sine_embed[..., self.d_model // 2:] *= (
                    refHW_cond[..., 0] / reference_points[..., 2]).unsqueeze(-1)
                query_sine_embed[..., :self.d_model // 2] *= (
                    refHW_cond[..., 1] / reference_points[..., 3]).unsqueeze(-1)
...
class MetaDINOTransformer(nn.Module):
    def __init__(self, d_model=256, nhead=8,
        ...
        self.image_query_ca_encoder = ImageQueryCAEncoder(decoder_layer, num_decoder_layers, decoder_norm,
                                                          return_intermediate=return_intermediate_dec,
                                                          d_model=d_model, query_dim=query_dim,
        ...
        assert two_stage_type == 'standard', "unknown param {} of two_stage_type".format(
            two_stage_type)
        ...
        self.cap_query_len = 100
        self.cap_query_feat = nn.Embedding(self.cap_query_len, d_model)
        self.cap_embed_feat = nn.Embedding(self.cap_query_len, 4)
        self.img_query2query_embed_proj = nn.Linear(768, d_model)
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None, fc_reg=None, fc_cls=None, fc_enc_reg=None, fc_enc_cls=None, vis_metas=None,
        ...
        if self.two_stage_type == 'standard':
            ...
            # [bs, \sum{hw}, 4] unsigmoid, output_proposlas maybe have inf value
            enc_outputs_coord_unselected = fc_enc_reg(
                output_memory) + output_proposals
            topk = self.num_queries
            # [bs, topk] is the index value
            topk_proposals = torch.topk(
                enc_outputs_class_unselected.max(-1)[0], topk, dim=1)[1]

            # gather boxes
            refpoint_embed_undetach = torch.gather(
                enc_outputs_coord_unselected, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, 4))  # unsigmoid [bs, topk, 4]
            refpoint_embed_ = refpoint_embed_undetach.detach()
            init_box_proposal = torch.gather(output_proposals, 1, topk_proposals.unsqueeze(
                -1).repeat(1, 1, 4)).sigmoid()  # sigmoid [bs, topk, 4]

            # gather tgt
            tgt_undetach = torch.gather(
                output_memory, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, self.d_model))

            tgt_cap = self.cap_query_feat.weight[None].repeat(bs, 1, 1)
            refpoint_embed_cap = self.cap_embed_feat.weight[None].repeat(
                bs, 1, 1)
            caption_text_query = self.image_query_ca_encoder(
                tgt=tgt_cap.transpose(0, 1),
                memory=output_memory.transpose(0, 1),
                memory_key_padding_mask=mask_flatten,
                pos=None,
                refpoints_unsigmoid=refpoint_embed_cap.transpose(0, 1),
                level_start_index=level_start_index,
                spatial_shapes=spatial_shapes,
                valid_ratios=valid_ratios,
            )

            caption_text_query_sum = torch.cat(
                [tgt_undetach, caption_text_query], dim=1)
            blip_loss = None
            captions = []
            if gt_caption is not None:
                blip_loss, image_feats, query_cap = self.captioner(
                    caption_text_query_sum,  gt_caption)
            else:
                captions, image_feats, query_cap = self.captioner.generate(
                    caption_text_query_sum)
            # 是否融合object 特征
            query_captioner_1 = caption_text_query.detach()
            query_captioner_2 = F.normalize(
                self.img_query2query_embed_proj(query_cap.detach()), dim=-1)
            query_captioner = torch.cat(
                [query_captioner_1, query_captioner_2], dim=1)
            memory_text = query_captioner
            memory_text_mask = torch.ones_like(
                memory_text[:, :, 0]) > 0
            extra['encoded_grounding'] = memory_text.float().permute(
                1, 0, 2)
            extra['grounding_attention_mask'] = ~memory_text_mask  # false 为自注意力区域

            if self.embed_init_tgt:
                tgt_ = self.tgt_embed.weight[:self.num_queries, None, :].repeat(
                    1, bs, 1).transpose(0, 1)  # [bs, topk, d_model]
            else:
                NotImplementedError
            ...
            refpoint_embed_ = self.refpoint_embed.weight[:self.num_queries, None, :].repeat(
        ...
        if self.two_stage_type == 'standard':
            ...
            # [1, 2, 900, 256], 这里只有two stage产生的topk的encoder embedding
            hs_enc = tgt_undetach.unsqueeze(0)
            ref_enc = refpoint_embed_undetach.sigmoid(
            ).unsqueeze(0)  # [1, 2, 900, 4]
        else:
            hs_enc = ref_enc = None
        ...
        # init_box_proposal: [2, 900, 4]
        # query_cap
        # image_feats
        if training:
            return hs, references, hs_enc, ref_enc, init_box_proposal, query_cap, blip_loss
        else:
            return hs, references, hs_enc, ref_enc, init_box_proposal, query_cap, captions
    ...
    def forward_with_query(self, srcs, masks, refpoint_embed_, pos_embeds, tgt_, attn_mask=None, fc_reg=None, fc_cls=None, fc_enc_reg=None, fc_enc_cls=None):
        ...
        # (num_consistency_query, bs, d_model)
        tgt = tgt_[:, None, :].repeat(1, bs, 1).transpose(0, 1)
        refpoint_embed = refpoint_embed_[:, None, :].repeat(
            1, bs, 1).transpose(0, 1)   # (num_consistency_query, bs, 4)
